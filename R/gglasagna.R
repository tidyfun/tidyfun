...
gglasagna <- function(data, y, order = NULL, label = NULL, 
  ...
  if (has_order_by) {
    ...
    order_by_value <- tf_eval %>% group_by(..y) %>% 
      summarize(..order_by_value = order_by(..fill)) %>% ungroup() %>% 
  ...
  if (!is.null(order_label) | !is.null(order_by_label)) {
    p <- p + labs(caption = paste("ordered by:", 
      paste0(order_label, ifelse(has_order_by & has_order, ";", ""), 
        order_by_label)))
  }
  if (order_ticks & is.discrete(pull(tf_eval, ..order))) {
    order_ticks <- data %>% arrange(desc(!!order)) %>%
      mutate(ticks = row_number()) %>% group_by(!!order) %>%
      summarize(tick_hi = max(ticks), tick_lo = min(ticks)) %>%
      mutate(label_pos = (tick_hi + tick_lo)/2,
        tick_pos = lead(tick_hi) + 0.5)
    p <- p +
      geom_hline(data = order_ticks, aes(yintercept = tick_pos),
