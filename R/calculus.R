...
derive_tfb_fpc <- function(f, order = 1, lower, upper, 
  ...
  new_basis <- if (order > 0) {
    ...
    tf_integrate(efunctions, lower = lower, upper = upper, definite = FALSE, 
      arg = arg)
  }  
...
tf_integrate.tfb <- function(f, lower = tf_domain(f)[1], upper = tf_domain(f)[2], 
  ...
  stopifnot(length(lower) %in% c(1, length(f)), 
    length(upper) %in% c(1, length(f)))
  if (definite) {
    return(tf_integrate(tfd(f, arg = arg), lower = lower, upper = upper, 
      arg = arg))
  }
  limits <- cbind(lower, upper)
  ...
  if (grepl("FPC", attr(f, "basis_label"))) {
    return(derive_tfb_fpc(f, order = -1, arg = arg, lower = lower, 
      upper = upper))
  }
}
