...
#' @importFrom stats approx coefficients
#' @importFrom glmnet cv.glmnet
new_tfb_wavelet <- function(data, domain = NULL, level = 2,
  ...
  # Look if every input grid is the same
  regular <- all(duplicated(arg_list)[-1])
  
  # interpolate the input grid to a dyadic, equispaced grid. If !regular a 
  # global grid is constructed
...
#' @describeIn tfb_wavelet convert data frames
...
#' @export
#' @describeIn tfb_wavelet convert matrices
tfb_wavelet.matrix <- function(data, arg = NULL, domain = NULL,
...
#' @export
#' @describeIn tfb_wavelet convert `tfd` (raw functional data)
tfb_wavelet.tfd <- function(data, arg = NULL, domain = NULL, resolution = NULL, 
...
#' @export
#' @describeIn tfb_wavelet convert `tfb`: modify wavelet basis or Lasso args.
tfb_wavelet.tfb <- function(data, domain = NULL, level = 2, arg = NULL,
                            resolution = NULL, filter_number = 5, 
                            penalized = FALSE, ...) {
  arg <- arg %||% tf_arg(data)
  resolution <- resolution %||% tf_resolution(data)
