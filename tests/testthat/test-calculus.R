...
cubic_b <- fbase(cubic, k = 45, bs = "tp")
square_irreg <- eval_irreg(expression(3 * x^2), g, domain)
square_b <- fbase(square, k = 45, bs = "tp")
...
test_that("deriv & integrate are reversible (approximately)", {
  ...
  expect_error(integrate(deriv(fbase(f)), definite = FALSE), "previously")
  
  f_pc <- fpcbase(f, smooth = FALSE)
