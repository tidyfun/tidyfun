...
expect_equivalent(tfd(list_reg, argvals = grid), f_reg)

list_irreg <- as.list(as.data.frame(t(mat_irreg))); names(list_irreg) <- 1:n
expect_equivalent(tfd(list_irreg, argvals = grid), f_irreg)

expect_equivalent(tfd(evaluations(f_irreg), argvals = argvals(f_irreg)), f_irreg)
