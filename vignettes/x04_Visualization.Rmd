...
Vector-valued functional data -- functions $f: \mathbb{R} \to \mathbb{R}^d$ stored as `tf_mv` vectors (see the `tf` package) -- can be mapped with the `tf` aesthetic just like univariate `tf` columns. `tf_ggplot()` picks a display that mirrors `tf`'s base-R `plot.tf_mv()`:
...
```{r mv-setup}
set.seed(1221)
# a 2-component tf_mv: two coordinates observed over the same argument grid
mv2 <- tfd_mv(list(x = tf_rgp(6), y = tf_rgp(6)))
mv_df <- tibble(id = factor(1:6), mv = mv2)
```

For `d == 2`, mapping the whole `tf_mv` column draws each curve's planar trajectory. **Use `geom_path()`, not `geom_line()`**: `geom_path()` connects points in argument (time) order, while `geom_line()` would re-sort by `x` and mangle any non-monotone curve.

```{r mv-trajectory}
mv_df |>
  tf_ggplot(aes(tf = mv, color = id)) +
...
```{r mv-txty}
tibble(fx = tf_rgp(6), fy = tf_rgp(6)) |>
  tf_ggplot(aes(tf_x = fx, tf_y = fy)) +
  geom_path()
```

For more than two components -- or to force value-vs-arg curves for a 2-component object via `type = "facet"` -- the data is unnested to a long form with a `.component` column. Add `facet_wrap(~ .component)` (or map `color = .component`) yourself:

```{r mv-facet}
mv3 <- tfd_mv(list(a = tf_rgp(4), b = tf_rgp(4), c = tf_rgp(4)))
tibble(id = factor(1:4), mv = mv3) |>
  tf_ggplot(aes(tf = mv, color = id)) +
  geom_line() +
