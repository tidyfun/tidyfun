...
## Lines, points and ribbons: `geom_spaghetti`, `geom_polpette` & `geom_papardelle`
...
A variant on the spaghetti plot is the polpette plot, which shows both the "noodles" (i.e. functional observations visualized as curves) and the "meatballs" (i.e. observed data values visualized as points). 
...
Ribbon plots for functional data are implemented via `geom_papardelle`, see the last example in the next section below.
...
dti_df |>
  ggplot() +
  geom_spaghetti(aes(tf = cca, col = case, alpha = 0.2 + 0.4 * (case == "control"))) +
  facet_wrap(~sex) +
...
## `geom_capellini` for spatial functional data

A flexible version of glyph plots (see [Wickham et al., 2012](https://doi.org/10.1002/env.2152), [`GGally::glyphs`](https://ggobi.github.io/ggally/articles/glyph.html)) for functional data is implemented in `geom_capellini`. 
To illustrate, we'll start with some data prep for the iconic Canadian Weather data:
...
Now we can plot a map of Canada with little sparkline-type boxes for annual temperature averages in red and precipitation in blue at the locations of the respective weather stations where the measurements were made: 

```{r}
ggplot(canada, aes(x = lon, y = lat)) +
  geom_capellini(aes(tf = precipl10),
    width = 4, height = 3, colour = "blue",
    line.linetype = 1
  ) +
  geom_capellini(aes(tf = temp),
    width = 4, height = 3, colour = "red",
    line.linetype = 1
  ) +
