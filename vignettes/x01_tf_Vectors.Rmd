...
vignette: >
  ...
  %\VignetteIndexEntry{Introducing tf vectors}
...
The code below creates some example data used in this vignette (and is explained in the [data conversion](https://fabian-s.github.io/tidyfun/articles/02_Conversion.html) vignette). 

First we extract two `tf` vectors from the `tidyfun::chf_df` dataset. These vectors contain fractional anisotropy tract profiles for the corpus callosum (`cca`) and the right corticospinal tract (`rcst`). When printed, `tf` vectors show the first few `arg` and `value` pairs for each subject.  
...
like the elements of the `rcst` vector.

```{r}
rcst[1:2]
rcst[1:2] %>% tf_arg() %>% str
rcst[1:2] %>% plot(pch = "x", col = viridis(2))
```
...
```{r}
(cca_five_fpc <- cca_five %>% tfb_fpc(smooth = FALSE, pve = .999))
(cca_five_fpc_smooth <- cca_five %>% tfb_fpc(pve = .95))
```
...
cca_five_fpc_smooth %>% lines(col = "blue")
```
...
cca_five %>% tf_zoom(0.2, 0.6) %>% tf_anywhere(value <= median(cca_five)[, arg])
```
