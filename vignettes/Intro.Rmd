...
ex %>% smooth("rollmedian", k = 5) %>% lines(col = pal_5)

ex %>% plot(alpha = .2, ylab = "Savitzky-Golay (quartic, 11 steps)")
ex %>% smooth("savgol", fl = 11) %>% lines(col = pal_5)
```
...
layout(t(1:3))
ex %>% plot(col = pal_5)
ex %>% smooth() %>% deriv() %>% plot(col = pal_5, ylab = "deriv(smooth(ex))")
ex %>% integrate(definite = FALSE) %>%  plot(col = pal_5)
```
```{r}
ex %>% integrate()
```
...
```{r, dti-fig2}  
gglasagna(dti, cca, 
  order = integrate(cca, definite = TRUE), arg = seq(0,1, l = 101)) + 
