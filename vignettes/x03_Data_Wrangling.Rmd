...
In many cases, the need for this operation can be avoided through suitable data wrangling prior to converting functional data to a `tf` object. It's also possible to `bind_rows` avoid by using `rbind`, which still works. See [tidyfun/issues/53](https://github.com/fabian-s/tidyfun/issues/53) for a discussion of related issues.  
