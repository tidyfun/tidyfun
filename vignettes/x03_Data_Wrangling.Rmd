...
The existing `mutate` function can be combined with several `tidyfun` functions, including `tf_smooth`, `tf_zoom`, and `tf_derive`.
...
dti_df |>
  ...
  tf_ggplot() +
  geom_line(aes(tf = cca_raw_deriv), alpha = 0.3, linewidth = 0.3, col = "blue") +
  geom_line(aes(tf = cca_tfb_deriv), alpha = 0.3, linewidth = 0.3, col = "red") +
  ylab("d/dt f(t)")
```
...
- Set `options(data.table.optimize = 0)`, which disables optimization, allowing the mean function to correctly dispatch for `tf` vectors.
- Alternatively, explicitly call `tf:::mean.tf(activity)` in your code to directly specify the correct `tf`-mean calculation.
