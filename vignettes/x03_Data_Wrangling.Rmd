...
```{r plot_chf}
chf_df[1:5,] %>% 
  ggplot(aes(tf = activity)) + geom_spaghetti(alpha = .1)
```
...
```{r plot_dti}
dti_df %>% 
  ggplot(aes(tf = cca)) + geom_spaghetti(alpha = .05)
```
...
dti_df %>% 
  ...
  ggplot(aes(tf = mean_rcst, color = case)) + 
...
chf_df %>% 
  ...
  ggplot(aes(tf = mean_act, color = day)) + 
  geom_spaghetti(alpha = .2) + 
  geom_spaghetti(aes(tf = smooth_mean), size = 2)
```
...
dti_df[1:10, ] %>%
  ...
  ggplot() + 
    geom_spaghetti(aes(tf = cca_raw_deriv), alpha = .3, lwd = .3, col = "blue") +
    geom_spaghetti(aes(tf = cca_tfb_deriv), alpha = .3, lwd = .3, col = "red") +
    ylab("d/dt f(t)")
```
