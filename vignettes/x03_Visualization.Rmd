...
#REMOVE THIS AND USE data-raw PREP INSTEAD
chf_df <- chf_df  %>% 
  select(-week) %>% 
...
chf_df %>% 
  filter(day %in% c("Mon")) %>% 
  gglasagna(activity, label = id)
```

Using `order`, you can sort the "layers" of the lasagna plot according to another column in your data -- for example, we could look at activity profiles sorted according to people's age:
```{r, eval = FALSE}
chf_df %>% 
  filter(day %in% c("Sat", "Sun")) %>% 
  gglasagna(activity, label = id, order = age)
```
If `order` is a discrete variable like `day`, it will add labels for the order variables to the plot:
