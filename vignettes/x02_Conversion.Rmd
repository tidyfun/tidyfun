...
dti_df %>% 
  ggplot() + 
  geom_spaghetti(aes(tf = cca, col = case, alpha = .2 + .4*(case == "control"))) +
  facet_wrap(~ sex) + 
...
temp_panel =
  canada %>% 
  ggplot(aes(tf = temp, color = region)) + geom_spaghetti() 

precip_panel =
  canada %>% 
  ggplot(aes(tf = precipl10, color = region)) + geom_spaghetti() 
