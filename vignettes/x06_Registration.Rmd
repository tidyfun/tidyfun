...
| Method | Use when | Avoid when | Template handling | Key arguments | Input grid | Noise robustness | Typical speed | Typical failure signs | First fallback |
|:--|:--|:--|:--|:--|:--|:--|:--|:--|:--|
| `srvf` (default) | Smooth, non-linear timing differences with shared shape family | Very noisy data, sparse grids, highly heterogeneous shapes | Implicit Karcher-type mean unless `template` supplied | `method = "srvf"`, `lambda` | regular only | Low (pre-smooth if noisy) | Fast (but O(n²) in grid) | Unstable warps, over-warping, inconsistent reruns | Pre-smooth inputs; try `affine` or `landmark` |
| `cc` | You want continuous-criterion alignment and can tune criterion | Complex phase-amplitude interactions or severe irregularity | Arithmetic mean unless `template` supplied | `method = "cc"`, `crit = 1/2`, `nbasis`, `lambda`, `conv`, `iterlim` | regular only | High (most stable) | Moderate | Sensitive to criterion choice, weak alignment gains | Change criterion, basis dimension and/or amount of penalization; Try `srvf` or `affine` |
| `affine` | Mostly shift/scale timing variability; need interpretable warps | Strong non-linear timing deformation | Arithmetic mean unless `template` supplied | `method = "affine"`, `type`, bounds | regular + **irregular** | Moderate | Fast | Residual misalignment of local features, boundary `NA`s under stronger shifts/scales | Set stricter/looser bounds; Upgrade to `srvf` |
| `landmark` | Reliable, repeated landmarks are available across curves | Ambiguous/noisy landmarks or mismatched counts | Uses `template_landmarks` (default: column means) | `method = "landmark"`, `landmarks`, `template_landmarks` | regular + **irregular** | Moderate; outlier-robust | Very fast | Forced/broken alignments from bad landmarks | Check appropriateness of landmarks; Use `srvf` or `affine` |
