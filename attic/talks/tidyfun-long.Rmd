---
title: '**`tf`& `tidyfun`**: Tidy Functional Data'
...
author: 
    ...
    - "Maximilian Mücke$^1$"
institute: 
    - "$^1$: LMU Munich -- Dept. of Statistics"
...
dti |>
  ...
  summarize(
    mean_cca = mean(cca),
    sd_cca = sd(cca),
    hi_cca = mean_cca + 2 * sd_cca,
    lo_cca = mean_cca - 2 * sd_cca
  ) |> 
  ggplot(aes(tf = mean_cca, color = sex, fill = sex)) +
  geom_spaghetti(alpha = 1, linewidth = 2) +
  geom_papardelle(aes(fmax = hi_cca, fmin = lo_cca), alpha = 0.2) +
  facet_grid(~ case) 
``` 
